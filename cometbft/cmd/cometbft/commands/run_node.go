...
func NewRunNodeCmd(nodeProvider nm.Provider) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if n.ControllerMediationEnabled() {
				signals := make(chan os.Signal, 1)
				signal.Notify(signals, os.Interrupt, syscall.SIGTERM)
				defer signal.Stop(signals)
				return waitForControllerMediationNode(n, signals)
			}
...
type controllerMediationNode interface {
	IsRunning() bool
	Stop() error
	Quit() <-chan struct{}
	FatalError() error
...
func waitForControllerMediationNode(n controllerMediationNode, signals <-chan os.Signal) error {
	select {
	case <-signals:
		if n.IsRunning() {
			if err := n.Stop(); err != nil {
				// A runtime fatal can win the stop race after IsRunning. In
				// that case the typed fatal below is the authoritative cause.
				if fatalErr := n.FatalError(); fatalErr != nil {
					return fatalErr
				}
				if n.IsRunning() {
