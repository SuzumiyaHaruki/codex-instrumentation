...
func testnetFiles(*cobra.Command, []string) error {
	...
	for i := 0; i < nValidators+nNonValidators; i++ {
		...
		if err := configureControllerMediationForTestnet(config, i); err != nil {
			return err
		}
...
func configureControllerMediationForTestnet(config *cfg.Config, index int) error {
	controller := config.ExperimentalController
	if controller == nil || !controller.Enabled {
		return nil
	}
	_, port, err := net.SplitHostPort(controller.CallbackListenAddress)
	if err != nil {
		return fmt.Errorf("invalid experimental controller callback listen address: %w", err)
	}
	controller.NodeID = index
	controller.CallbackAdvertiseAddress = net.JoinHostPort(hostnameOrIP(index), port)
	if err := controller.ValidateBasic(); err != nil {
		return fmt.Errorf("invalid generated experimental controller config for node %d: %w", index, err)
	}
	return nil
}
