...
type Config struct {
	...
	ExperimentalController *ControllerMediationConfig `mapstructure:"experimental_controller"`
}
...
func DefaultConfig() *Config {
	return &Config{
		...
		ExperimentalController: DefaultControllerMediationConfig(),
	}
}
...
func (cfg *Config) ValidateBasic() error {
	...
	if err := cfg.ExperimentalController.ValidateBasic(); err != nil {
		return ErrInSection{Section: "experimental_controller", Err: err}
	}
...
// -----------------------------------------------------------------------------
// ControllerMediationConfig

const (
	controllerMaxNativePayload = 16_000_000
	controllerAdapterOverhead  = 6
	controllerJSONOverhead     = 512
)

// ControllerMediationConfig configures the opt-in, controller-mediated P2P
// delivery mode. The mode is intended only for isolated test networks.
type ControllerMediationConfig struct {
	Enabled                  bool          `mapstructure:"enabled"`
	ControllerAddress        string        `mapstructure:"controller_address"`
	CallbackListenAddress    string        `mapstructure:"callback_listen_address"`
	CallbackAdvertiseAddress string        `mapstructure:"callback_advertise_address"`
	NodeID                   int           `mapstructure:"node_id"`
	OutboundQueueCapacity    int           `mapstructure:"outbound_queue_capacity"`
	OutboundQueueBytes       int64         `mapstructure:"outbound_queue_bytes"`
	CallbackQueueCapacity    int           `mapstructure:"callback_queue_capacity"`
	CallbackQueueBytes       int64         `mapstructure:"callback_queue_bytes"`
	CallbackMaxBodyBytes     int64         `mapstructure:"callback_max_body_bytes"`
	CounterRegistryCapacity  int           `mapstructure:"counter_registry_capacity"`
	RegistrationAttempts     int           `mapstructure:"registration_attempts"`
	RegistrationRetryDelay   time.Duration `mapstructure:"registration_retry_delay"`
	RequestTimeout           time.Duration `mapstructure:"request_timeout"`
	SendTimeout              time.Duration `mapstructure:"send_timeout"`
	ShutdownTimeout          time.Duration `mapstructure:"shutdown_timeout"`
}

// DefaultControllerMediationConfig returns the disabled controller-mediation
// configuration. Operational limits are populated so enabling only requires
// identity and address settings.
func DefaultControllerMediationConfig() *ControllerMediationConfig {
	return &ControllerMediationConfig{
		Enabled:                 false,
		OutboundQueueCapacity:   1024,
		OutboundQueueBytes:      67_108_864,
		CallbackQueueCapacity:   256,
		CallbackQueueBytes:      67_108_864,
		CallbackMaxBodyBytes:    22_000_000,
		CounterRegistryCapacity: 65_536,
...
// ValidateBasic validates the enabled mediation configuration. Disabled mode
// intentionally ignores incomplete address and identity settings.
func (cfg *ControllerMediationConfig) ValidateBasic() error {
	if cfg == nil {
		return nil
	}
	if !cfg.Enabled {
	...
	for name, address := range map[string]string{
		"controller_address":         cfg.ControllerAddress,
		"callback_listen_address":    cfg.CallbackListenAddress,
		"callback_advertise_address": cfg.CallbackAdvertiseAddress,
	} {
		if address == "" {
	...
	if cfg.NodeID < 0 {
		return errors.New("node_id must be non-negative")
	}
	for name, value := range map[string]int{
		"outbound_queue_capacity":   cfg.OutboundQueueCapacity,
		"callback_queue_capacity":   cfg.CallbackQueueCapacity,
		"counter_registry_capacity": cfg.CounterRegistryCapacity,
		"registration_attempts":     cfg.RegistrationAttempts,
	} {
		if value <= 0 {
			return fmt.Errorf("%s must be positive", name)
		}
	}
	minPayloadBytes := int64(controllerAdapterOverhead + controllerMaxNativePayload)
	if cfg.OutboundQueueBytes < minPayloadBytes {
		return fmt.Errorf("outbound_queue_bytes must be at least %d", minPayloadBytes)
	}
	if cfg.CallbackQueueBytes < minPayloadBytes {
		return fmt.Errorf("callback_queue_bytes must be at least %d", minPayloadBytes)
	}
	minBodyBytes := int64(base64.StdEncoding.EncodedLen(int(minPayloadBytes)) + controllerJSONOverhead)
	if cfg.CallbackMaxBodyBytes < minBodyBytes {
		return fmt.Errorf("callback_max_body_bytes must be at least %d", minBodyBytes)
	}
	for name, value := range map[string]time.Duration{
		"registration_retry_delay": cfg.RegistrationRetryDelay,
	...
	if cfg.RegistrationRetryDelay > 2*time.Second {
		return errors.New("registration_retry_delay must not exceed 2s")
	}
	if cfg.SendTimeout > 10*time.Second {
		return errors.New("send_timeout must not exceed the native 10s send bound")
	}
	return nil
}
