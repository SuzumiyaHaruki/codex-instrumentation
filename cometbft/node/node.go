...
type Node struct {
	...
	controllerMediation *controllerMediation
	controllerLifecycle sync.Mutex
	controllerFatalMu   sync.Mutex
	controllerFatal     error
}
...
func (n *Node) OnStart() error {
	n.controllerLifecycle.Lock()
	defer n.controllerLifecycle.Unlock()
	mediationStarted := false
	startComplete := false
	defer func() {
		if mediationStarted && !startComplete {
			n.sw.SetMediatedMessageSink(nil)
			n.controllerMediation.Stop()
			n.controllerMediation = nil
		}
	}()
	now := cmttime.Now()
	...
	if n.config.ExperimentalController != nil && n.config.ExperimentalController.Enabled {
		if err := n.config.ExperimentalController.ValidateBasic(); err != nil {
			return fmt.Errorf("invalid experimental_controller configuration: %w", err)
		}
		registry, err := n.sw.FreezeNativeRoutes(selectedControllerRoutes)
		if err != nil {
			return fmt.Errorf("freeze controller mediation routes: %w", err)
		}
		n.controllerMediation = newControllerMediation(
			n.config.ExperimentalController,
			n.nodeKey.ID(),
			registry,
			n.sw.Peers(),
			n.p2pMetrics,
			n.Logger.With("module", "controller_mediation"),
			n.recordControllerFatal,
		)
		if err := n.controllerMediation.Start(); err != nil {
			n.controllerMediation = nil
			return fmt.Errorf("start controller mediation: %w", err)
		}
		mediationStarted = true
		n.sw.SetMediatedMessageSink(n.controllerMediation)
	}
...
// ControllerMediationEnabled reports whether this node owns the opt-in service.
func (n *Node) ControllerMediationEnabled() bool {
	return n.config != nil && n.config.ExperimentalController != nil && n.config.ExperimentalController.Enabled
}

// FatalError returns the first process-visible controller-mediation fatal cause.
func (n *Node) FatalError() error {
	n.controllerFatalMu.Lock()
	defer n.controllerFatalMu.Unlock()
	return n.controllerFatal
}

func (n *Node) recordControllerFatal(err error) {
	n.controllerFatalMu.Lock()
	if n.controllerFatal == nil {
		n.controllerFatal = err
	}
	n.controllerFatalMu.Unlock()
	go func() {
		if n.IsRunning() {
			if stopErr := n.Stop(); stopErr != nil {
				n.Logger.Error("Failed to stop after controller mediation fatal", "err", stopErr)
			}
		}
	}()
}
