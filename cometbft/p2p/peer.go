...
func (p *peer) Send(e Envelope) bool {
	return p.send(e.ChannelID, e.Message, SendModeBlocking, p.mconn.Send)
}
...
func (p *peer) TrySend(e Envelope) bool {
	return p.send(e.ChannelID, e.Message, SendModeNonBlocking, p.mconn.TrySend)
}

func (p *peer) send(chID byte, msg proto.Message, mode SendMode, sendFunc func(byte, []byte) bool) bool {
	if !p.IsRunning() {
		return false
	...
	nativeMsg := msg
	selected := p.mediation != nil && p.mediation.Selected(chID, nativeMsg)
	...
	if err != nil {
		...
		if selected {
			p.mediation.LocalInvariant(fmt.Errorf("encoding selected channel %X: %w", chID, err))
		}
		return false
	}
	var res bool
	if selected {
		res = p.mediation.Submit(p.ID(), chID, nativeMsg, msgBytes, mode)
	} else {
...
func createMConnection(
	...
	onReceive := func(chID byte, msgBytes []byte) {
		if p.mediation != nil && p.mediation.SelectedRoute(chID) {
			p.mediation.DirectInboundRejected(p.ID(), chID)
			return
		}
