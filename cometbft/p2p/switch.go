...
type Switch struct {
	...
	metrics     *Metrics
	mediationMu sync.RWMutex
	mediation   MediatedMessageSink
}
...
// SetMediatedMessageSink installs or removes the mediation boundary for peers
// accepted or dialed concurrently with lifecycle transitions.
func (sw *Switch) SetMediatedMessageSink(sink MediatedMessageSink) {
	sw.mediationMu.Lock()
	defer sw.mediationMu.Unlock()
	sw.mediation = sink
}

func (sw *Switch) mediatedMessageSink() MediatedMessageSink {
	sw.mediationMu.RLock()
	defer sw.mediationMu.RUnlock()
	return sw.mediation
}

// FreezeNativeRoutes copies selected route bindings after construction-time
// reactor options and before Switch startup.
func (sw *Switch) FreezeNativeRoutes(selected []byte) (*NativeRouteRegistry, error) {
	routes := make(map[byte]NativeRoute, len(selected))
	for _, channelID := range selected {
		if _, duplicate := routes[channelID]; duplicate {
			return nil, fmt.Errorf("duplicate selected channel %X", channelID)
		}
		prototype := sw.msgTypeByChID[channelID]
		reactor := sw.reactorsByCh[channelID]
		if prototype == nil || reactor == nil {
			return nil, fmt.Errorf("selected channel %X has incomplete native binding", channelID)
		}
		capacity := 0
		for _, descriptor := range sw.chDescs {
			if descriptor.ID == channelID {
				capacity = descriptor.FillDefaults().RecvMessageCapacity
				break
			}
		}
		if capacity <= 0 {
			return nil, fmt.Errorf("selected channel %X has no receive capacity", channelID)
		}
		routes[channelID] = NativeRoute{
			ChannelID:       channelID,
			ReceiveCapacity: capacity,
			prototype:       proto.Clone(prototype),
			reactor:         reactor,
		}
	}
	return newNativeRouteRegistry(routes, sw.StopPeerForError), nil
}
...
func (sw *Switch) acceptRoutine() {
	for {
		p, err := sw.transport.Accept(peerConfig{
			...
			metrics:       sw.metrics,
			mediation:     sw.mediatedMessageSink(),
